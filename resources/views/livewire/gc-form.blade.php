<div>
    ...
    <div class="text-center p-3 mx-auto">
        ...
        <x-jet-button wire:click="showCreateGcModal">
            New General Contractor (GC)
        </x-jet-button>
    </div>

    <!-- Gc Card: We will insert for each statement here. ----------------------------------------------------------------------->
    <div>
                ...
                <div class="w-full flex items-center justify-between p-6 space-x-6">
                    <div class="flex-1 truncate">
                        <div class="flex items-center space-x-3">
                            <h3 class="text-gray-900 text-sm font-medium truncate">{{ $gc->name }}</h3>
                            <span class="flex-shrink-0 inline-block px-2 py-0.5 text-green-800 text-xs font-medium bg-green-100 rounded-full">{{ $gc->status }}</span>
                        </div>
                        <p class="mt-1 text-gray-500 text-sm truncate">Street: {{ $gc->street }}</p>
                        <p class="mt-1 text-gray-500 text-sm truncate">City: {{ $gc->city }} </p>
                        <p class="mt-1 text-gray-500 text-sm truncate">State: {{ $gc->state }}</p>
                        <p class="mt-1 text-gray-500 text-sm truncate">Zip: {{ $gc->zip }}</p>
                        <p class="mt-1 text-gray-500 text-sm truncate">Contact: {{ $gc->contact }}</p>
                        <p class="mt-1 text-gray-500 text-sm truncate">Phone: {{ $gc->phone }}</p>
                        <p class="mt-1 text-gray-500 text-sm truncate">Email: {{ $gc->email }}</p>
                        <p class="mt-1 text-gray-500 text-sm truncate">Website: {{ $gc->website }}</p>
                        <p class="mt-1 text-gray-500 text-sm truncate">Description: {{ $gc->Description }}</p>
                ...
                <div>
                    <div class="-mt-px flex divide-x divide-gray-200">
                        <div class="w-0 flex-1 flex">
                            <a href="mailto:{{ $gc->email }}" class="relative -mr-px w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-gray-700 font-medium border border-transparent rounded-bl-lg hover:text-gray-500">
                        ...
                        <div class="-ml-px w-0 flex-1 flex">
                            <a href="tel:+1-{{ $gc->phone }}" class="relative w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-gray-700 font-medium border border-transparent rounded-br-lg hover:text-gray-500">
    ...
    <!-- Modal Form for gc Entry ------------------------------------------------------------------------------------------------>

    @csrf
    <x-jet-dialog-modal wire:model="showModalForm">

        <!-- Modal Header -->
        <x-slot name="title">Create Gc</x-slot>
        <x-slot name="content">
            ...
            <form class="w-full max-w-sm">
                ...
                <div class="flex flex-wrap -mx-3 mb-2">
                    ...
                    <div class="w-full px-5 mb-6">
                        ...
                        <input wire:model.lazy="name" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-name" name="grid-name" type="text" placeholder="New Gc">
                    </div>
    ...
    <!-- Modal Footer -->
        <x-slot name="footer">
            <x-jet-button wire:click="storeGc">Save General Contractor</x-jet-button>
        </x-slot>
